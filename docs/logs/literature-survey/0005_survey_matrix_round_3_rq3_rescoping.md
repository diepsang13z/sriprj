...
| R3-S1 | Li, Zhu, Zhang, Liu, & Yu (2024), *JTAER*. [DOI](https://doi.org/10.3390/jtaer19010015) | Online product reviews, ngoài hospitality | Kano utility + two-stage nonlinear satisfaction model | Basic attributes được xét ở non-compensatory stage; các thuộc tính còn lại mới đi vào compensatory utility stage | Cung cấp cấu trúc toán học rõ nhất cho RQ rộng hơn; chỉ dùng làm supporting method vì khác domain | Level VI; peer-reviewed OA; đã kéo và lưu trữ PDF toàn văn tại `refs/asymmetric-compensation/2024_li_two_stage_satisfaction_decision_model.pdf` |
...
> **Nguyên tắc:** Không nên xóa file đã tải; toàn bộ file cũ/lệch hướng (Topçu, McMurry, Patil, Puh) đã được chuyển về thư mục `refs/legacy/` để giữ trọn vẹn audit trail.
