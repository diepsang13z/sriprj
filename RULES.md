...
- **Nhật ký là bản chụp, không phải bản nháp.** Tệp ghi *việc đã xảy ra* — `docs/logs/progress/`, `docs/logs/brainstorm/`, và ma trận khảo sát theo vòng ở `docs/logs/literature_survey/` — là **bất biến sau khi ghi**: không sửa, không xoá nội dung cũ, kể cả khi nội dung đó sai. Cái *đang* đúng chỉ sống ở tài liệu sống (`RDR`, `STATUS.md`, `BACKLOG.md`, `notes/`, `refs/*/SOURCES.md`, `reports/`), và chỉ ở đó mới được sửa.
- **Đính chính bằng cách thêm, không bằng cách sửa.** Giữ nguyên dòng cũ, chèn ngay dưới nó một dòng đủ bốn thứ: **giá trị cũ · giá trị mới · ngày · căn cứ** (`RDR` hoặc log kiểm chứng). Nhật ký **trỏ** tới số, không sở hữu số.
- **Lý do, tiêu chí và quyết định thì không bao giờ sửa**, chỉ đính chính. Nếu một quyết định đã ra dựa trên số sai và tính hợp lệ của nó đổi theo, phải mở `RDR` mới — không được lặng lẽ thay số.
- **Ngoại lệ một lần, ghi lại để không ai tưởng đó là chuẩn:** đợt đính chính ngày 2026-09-23 theo `RDR-0006` — 12 dòng ở 8 tệp nhật ký — làm bằng cách sửa tại chỗ, vì luật này ra đời sau đó; hai chỗ là "nhà" của con số có ghi kèm giá trị cũ, các chỗ còn lại tra ở `RDR-0006` mục 1 và 2.2 cùng git history. Từ đây về sau dùng cách chèn dòng.
